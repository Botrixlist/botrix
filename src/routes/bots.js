...
route.use('/:id', async (req, res, next) => {
    ...
    await bots.updateOne({botid: req.params.id}, {$set: { views:  amountOfViews}})
    let b;
    let person;
    try {
        person = await req.app.get('client').guilds.cache.get(config.GUILD_ID).members.fetch(bot.owners[0]);
        let c = await req.app.get('client').guilds.cache.get(config.GUILD_ID).members.fetch(bot.botid);
    ...
    if(req.user){
        ...
        if(userObject){
            await user.updateOne({ userid: req.user.id },  {$push: {  recentViews: bot } });
        }
    }
    ...
    const ordered = {};
    Object.keys(badgesDict).sort().forEach(function(key) {
